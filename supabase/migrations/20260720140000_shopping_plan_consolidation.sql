-- Fase 6: los ingredientes faltantes de una receta planificada se consolidan en
-- Compra como sección «Para el plan», sin duplicar ni pisar lo manual.
...
alter table public.shopping_items
  ...
  check (source in ('manual', 'pantry', 'plan'));

-- La cantidad es opcional: un producto sigue siendo útil en la lista aunque no
-- sepamos cuánto hace falta.
alter table public.shopping_items
  add column quantity numeric check (quantity is null or quantity > 0),
  add column unit_code text
    check (unit_code is null or unit_code in ('unit', 'g', 'kg', 'ml', 'l'));

-- Dos cantidades solo se suman si miden lo mismo. `null` = dimensión desconocida,
-- que nunca se considera compatible.
create function private.unit_dimension(unit_code text)
returns text language sql immutable set search_path = '' as $$
  select case
    when unit_code in ('g', 'kg') then 'mass'
    when unit_code in ('ml', 'l') then 'volume'
    when unit_code = 'unit' then 'count'
  end;
$$;

create function private.unit_to_base(quantity numeric, unit_code text)
returns numeric language sql immutable set search_path = '' as $$
  select case when unit_code in ('kg', 'l') then quantity * 1000 else quantity end;
$$;

create function private.unit_from_base(quantity numeric, unit_code text)
returns numeric language sql immutable set search_path = '' as $$
  select case when unit_code in ('kg', 'l') then quantity / 1000 else quantity end;
$$;
...
create function public.shopping_add_plan_items(items jsonb, idempotency_key text)
...
revoke all on function public.shopping_add_plan_items(jsonb, text) from public, anon, authenticated;
grant execute on function public.shopping_add_plan_items(jsonb, text) to authenticated;
