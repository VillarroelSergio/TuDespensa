...
export function RecipesLibrary({ initialRecipes }: { initialRecipes: Recipe[] }) {
  ...
  async function handleLoadSeed() {
    if (pending) return
    setPending(true); setStatus('')
    try { const result = await loadSeed(); setStatus(result.loaded > 0 ? `Añadimos ${result.loaded} recetas base.` : 'Ya tenías las recetas base.'); refresh() }
    catch { setStatus('No hemos podido cargar las recetas base.') }
