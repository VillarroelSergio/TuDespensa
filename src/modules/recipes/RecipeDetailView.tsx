...
export function RecipeDetailView({ recipe }: { recipe: RecipeDetail }) {
  ...
  return <main className="recipe-detail">
    <a className="recipe-back" href="/recetas">← Recetas</a>
    ...
    <section aria-labelledby="detail-ingredients">
      ...
      {recipe.ingredients.length
        ? <ul className="recipe-detail__ingredients">{recipe.ingredients.map((ingredient) => <li key={ingredient.position}>{formatIngredient(ingredient)}</li>)}</ul>
    ...
    <section aria-labelledby="detail-steps">
      ...
      {recipe.steps.length
        ? <ol className="recipe-detail__steps">{recipe.steps.map((step) => <li key={step.position}>{step.instruction}</li>)}</ol>
        : <p className="recipes-empty">Aún no hay pasos.</p>}
    </section>

    {/* ponytail: «Añadir al plan» es la integración con Plan (Fase 5); aquí solo editar. */}
    <a className="recipe-detail__edit" href={`/recetas/${recipe.id}/editar`}>{recipe.status === 'pending' ? 'Revisar receta' : 'Editar'}</a>
  </main>
}
