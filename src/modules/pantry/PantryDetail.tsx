...
const approximateOptions: Array<{
  value: PantryApproximateState
  label: string
}> = [
...
export function PantryDetail({ item, onClose, onSave }: Props) {
  const [trackingMode, setTrackingMode] = useState<PantryTrackingMode>(
    item.trackingMode,
  )
  const [quantity, setQuantity] = useState(String(item.quantity ?? ''))
  const [unitCode, setUnitCode] = useState<PantryUnitCode>(
    item.unitCode ?? 'unit',
  )
  const [approximateState, setApproximateState] =
    useState<PantryApproximateState>(item.approximateState ?? 'some')
  ...
  return (
    <aside className="pantry-detail" aria-label={`Detalle de ${item.name}`}>
      ...
      <div className="pantry-detail__title">
        <h2>{item.name}</h2>
        <span
          className={`pantry-detail__status pantry-detail__status--${item.status}`}
        >
          {item.status === 'out'
            ? 'Se terminó'
            : item.status === 'low'
      ...
      <section className="pantry-detail__section">
        ...
        <div className="pantry-detail__chips">
          {trackingOptions.map((option) => (
            <button
              className={
                trackingMode === option.value ? 'is-selected' : undefined
              }
              type="button"
              key={option.value}
      ...
      {trackingMode === 'approximate' ? (
        <section className="pantry-detail__section">
          ...
          <div className="pantry-detail__chips">
            {approximateOptions.map((option) => (
              <button
                className={
                  approximateState === option.value ? 'is-selected' : undefined
                }
                type="button"
                key={option.value}
        ...
        <section className="pantry-detail__section">
          ...
          <div className="pantry-detail__quantity">
            <button
              type="button"
              aria-label={`Restar a ${item.name}`}
              onClick={() => adjust(-1)}
            >
              −
            </button>
          ...
          {trackingMode === 'measure' ? (
            <label className="pantry-detail__unit">
              ...
              <select
                value={unitCode}
                onChange={(event) =>
                  setUnitCode(event.target.value as PantryUnitCode)
                }
              >
      ...
      <button
        ...
        disabled={
          pending || (quantityValue !== null && Number.isNaN(quantityValue))
        }
        type="button"
        onClick={() => void save()}
      >
      ...
      <p className="pantry-detail__secondary">
        Movimientos recientes, recetas relacionadas y corrección avanzada
        aparecerán aquí.
      </p>
    </aside>
  )
