...
- **Fase 8 (cocinar y consumo asistido) implementada** en `feature/cook-and-consume` (migración `20260721110000_cook_and_consume.sql`): `planned_meals` gana `cooked_at`/`cooked_by` (una comida se cocina como mucho una vez, sin tabla nueva) y la RPC idempotente `plan_cook_meal(meal_date, meal_type, consumptions, idempotency_key)` marca el hueco como cocinado y aplica los descuentos confirmados como movimientos `consumption` trazables (`item_snapshot.source = 'meal_cooked'` + `recipe_id`). `consumptions` es el **estado resultante** de cada producto (lo que la persona confirmó o corrigió); la RPC valida la forma, exige que el modo no cambie y que una cantidad nunca suba (cocinar solo consume), y controla versión por ítem: si otro integrante cambió un producto o ya cocinó la comida, aborta entero (`serialization_failure`→CONFLICT) para recargar sin descontar a medias; el patrón `pantry_claim`/`pantry_store_result` da idempotencia por lote. UI: en P1 el menú del hueco añade **Marcar como cocinada** (oculto si ya está cocinado) → nueva vista `/plan/cocinar` (`CookReview`), que empareja ingredientes↔despensa con la misma regla que las sugerencias (`matches` de `suggestions.ts`, ahora exportado), propone un descuento por producto (cantidad si las unidades miden lo mismo, bajar un nivel si es aproximado) y deja **confirmar o corregir** cada línea antes de tocar la Despensa; el refresco Realtime de `planned_meals`/`pantry_items` recarga la revisión en vivo, el hueco cocinado muestra insignia «✓ Cocinada» y el aviso `?cocinada={n}` → `cookNotice` → «Cocinada · hemos descontado {n} producto(s) de tu despensa». Módulo puro `src/modules/plan/cooking.ts` (`buildCookLines`, `buildConsumptions`, conversión de unidades g/ml como los helpers de BD) reutilizado por la acción y el componente. Test SQL 8 (descuento de medida + baja de nivel, marca de cocinado, movimiento trazable, idempotencia sin doble descuento, ya-cocinado y conflicto de versión abortan sin aplicar a medias, aislamiento entre hogares) y tests unitarios de `cooking`. **Deltas conscientes:** el emparejamiento sigue siendo por subcadena normalizada (sin catálogo canónico); el descuento propuesto no escala por raciones; cocinar no convierte un producto de `approximate` a `measure` aunque la receta traiga cantidad; un ingrediente sin producto en despensa no aparece (su falta se resuelve en Compra, Fase 6); marcar cocinada no es reversible (una corrección posterior es una edición de despensa).
...
- **Arquitectura:** Fases 4A–4C, 5A, 5B, 5C, 6, 7 y 8 cerradas. Con la Fase 8 el recorrido principal **planificar → comprar → confirmar → cocinar/consumir** queda completo de extremo a extremo. Sigue C4 (captura de ticket de compra: OCR/importación de líneas detectadas, nodos Figma `COMPRA` 29:3999/29:4136) y la Fase 9 (verificaciones bajo demanda + cobertura E2E automatizada, incluida la del ciclo completo, antes de declarar los flujos completos).
