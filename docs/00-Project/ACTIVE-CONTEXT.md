...
- **Extensión de taxonomía (2026-07-28):** `main_ingredient` usa el vocabulario curado Pasta, Verduras, Legumbres, Arroz y cereales, Huevo, Aves, Carne y Pescado al editar una receta. La biblioteca y el selector de Plan muestran filtros rápidos para Pasta, Verduras, Legumbres, Carne y Pescado, con coincidencia sin distinción de mayúsculas o tildes; las categorías restantes siguen disponibles en el filtro avanzado. En Plan, la búsqueda y el filtro viajan en la URL para conservar la intención al navegar.
- **Ajustes UX de Plan (2026-07-28):** la agenda prioriza móvil (huecos apilados; carriles desde tablet) y las acciones de una receta se abren en un diálogo nativo: hoja inferior en móvil y diálogo centrado en tablet/escritorio. Conserva foco modal, Escape y un cierre explícito; todos los controles principales alcanzan 44 px.
- **Selector de Plan móvil (2026-07-28):** los filtros por ingrediente se desplazan horizontalmente en móvil (sin aumentar innecesariamente la altura), el selector comunica el número de recetas disponibles y cada tarjeta muestra la acción «Elegir» sin perder que toda la tarjeta es pulsable. Elegir abre una confirmación accesible de la receta, el hueco y las raciones: hoja inferior en móvil y diálogo centrado desde tablet, con cierre explícito y controles de 44 px.
- **Raciones en Compra (2026-07-28):** al confirmar una receta desde P2, las cantidades cuantificadas que se consolidan en Compra se escalan con la proporción `raciones elegidas / raciones base de la receta`; si falta cualquiera de ambas, se conserva la cantidad de la receta. El ajuste posterior de raciones no modifica una lista ya consolidada.
- **Revisión de cocina móvil (2026-07-28):** cada descuento cuantificado comunica en línea la cantidad que quedará en despensa; desmarcar una línea informa que no se modificará. Así se confirma el resultado, no solo el delta, antes de cocinar.
- **Resumen semanal de Plan (2026-07-28):** el progreso usa lenguaje explícito («1 de 14 comidas planificadas») en lugar de la fracción ambigua «1/14 comidas decididas».
- **Seguridad de acciones de Plan (2026-07-28):** eliminar una comida requiere una segunda confirmación dentro del diálogo móvil; los filtros activos se comunican con `aria-current="page"`.
- **Fase 5A (plan semanal base) implementada** en `feature/weekly-plan-core` (migración `20260720130000_weekly_plan_core.sql`): `meal_plans` (una semana por hogar, identificada por su lunes ISO, creada de forma perezosa) y `planned_meals` con **unicidad por hogar, fecha y servicio** (`lunch`/`dinner`), RLS de solo lectura para miembros activos y Realtime. RPCs idempotentes `plan_set_meal` (valida servicio y raciones, exige que la receta sea del hogar, y reasignar un hueco sustituye la receta sin duplicarla) y `plan_clear_meal` (vaciar un hueco ya vacío devuelve `removed = 0` sin error, para que el «Deshacer» de 5B se pueda reintentar). Vista P1 (`WeekView`, componente de servidor): cabecera con rango de semana y navegación anterior/siguiente por enlace `?semana=YYYY-MM-DD` (sin estado de cliente), resumen `{n} de 14 comidas planificadas`, siete días en orden temporal con huecos Comida/Cena siempre presentes, texto guía una sola vez con el plan vacío y «Volver a esta semana» al navegar. Responsive **móvil primero**: agenda de siete filas con huecos apilados en móvil, carriles Comida/Cena desde tablet y proporciones ampliadas solo en escritorio. Test SQL 5A (idempotencia, lunes ISO, sustitución de hueco, comida/cena independientes, servicio/raciones inválidos, limpieza idempotente, aislamiento entre hogares) y test unitario del cálculo de semana (`weekStart`, cruce de mes/año, rangos y conteo). **Deltas conscientes:** el hueco vacío muestra su única acción **Añadir** deshabilitada porque el selector P2 es Fase 5B; el menú contextual (cambiar receta, ajustar raciones, mover, eliminar con Deshacer) también es 5B; las fechas se calculan en UTC a partir de `YYYY-MM-DD` para que el día no se desplace por zona horaria.
- **Fase 5B (selección y edición del plan) implementada** en `feature/weekly-plan-selection` (consume las RPC `plan_set_meal` y `plan_clear_meal` de 5A, ampliadas con `plan_move_meal` en la migración `20260728111002_plan_move_meal_atomic.sql`). Vista P2 independiente en `/plan/elegir?fecha=&servicio=` (`ChooseRecipeView`): cabecera de retorno `Añadir a · martes, comida`, H1 «¿Qué quieres comer?», biblioteca guardada con búsqueda por título y estados «sin recetas guardadas» (enlace a Recetas) y «sin resultados» (ver todas). Elegir una receta confirma el hueco directamente y vuelve a la semana; abandonar P2 no modifica nada porque no hay borrador. En P1, el hueco vacío enlaza a P2 y el hueco ocupado abre un menú contextual (`<details>`, sin JavaScript de cliente) con **Cambiar receta**, **Ajustar raciones**, **Mover** (`<input type="date">` + servicio) y **Eliminar**, que pide confirmación en un segundo `details` y deja un banner **Deshacer**. Server actions `assignMealAction` / `moveMealAction` / `removeMealAction` validan hueco, receta y raciones antes de llamar a la RPC. Tests unitarios de `slotLabel` y `parseUndo`. **Deltas conscientes:** las tres sugerencias explicables con motivo y disponibilidad son Fase 5C (aquí se lista la biblioteca con búsqueda); la consolidación de faltantes en Compra y su confirmación `Hemos añadido {n} productos` son Fase 6; el estado para deshacer viaja en la URL (`?deshacer=fecha:servicio:receta:raciones`) en vez de en un registro de deshacer; el movimiento es atómico mediante `plan_move_meal`: conserva el origen si el destino ya está ocupado y no toca Compra, porque no cambian los ingredientes planificados.
...
- **Calibración OCR del piloto (2026-07-25):** el procesamiento continúa íntegramente en el navegador y sin persistir la imagen. Antes de OCR se realza localmente el contraste y se usa segmentación de bloque para conservar las filas de un ticket; el parser elimina importes y grupos de IVA de la columna derecha, ignora descuentos/totales y asocia el peso o las unidades que el supermercado imprime en la línea previa al artículo. Todo resultado sigue siendo editable antes de importarlo. Quedan cubiertos con pruebas unitarias los patrones observados en el ticket real del piloto.
- **PDF y galería para tickets (2026-07-25):** C4 permite elegir una imagen de la galería, hacer una foto o seleccionar un PDF desde el móvil. Los PDF electrónicos se interpretan por su texto embebido, ordenado por filas y columnas; un PDF escaneado renderiza sus páginas y usa el OCR local existente. Se amplía el parser para filas de ticket digital con cantidad y columnas de precio sin símbolo de euro. Ningún archivo se sube ni se persiste.
