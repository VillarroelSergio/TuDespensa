...
- **Fase 9 (cierre de calidad) en curso** en `feature/fase9-verification-gate` (PR #9) y `feature/e2e-quality-gate` (PR #8, arnés E2E de onboarding). Puerta de calidad ejecutada de verdad y en verde tras sanear la deuda de verificación diferida de la app (**33 errores de tipos + 7 de estilo**, sin cambiar comportamiento; incluida una desincronización real de los tipos de `shopping_items`): `lint` 0, `tsc` 0, `vitest` 82/82, `build` 13 rutas, y **E2E de dos sesiones** de onboarding (Auth+RLS+Realtime) 2/2. Evidencia en [[FASE9-VERIFICATION-EVIDENCE]]. **Pendiente:** E2E del ciclo `planificar → comprar → cocinar`, revisión manual responsive a 390/768/1440 px, fusión a `develop` de #8 y #9 (la realiza la persona responsable) y cierre de Notion.
...
- **Arquitectura:** Fases 4A–4C, 5A, 5B, 5C, 6, 7 y 8 cerradas; el recorrido `planificar → comprar → cocinar/consumir` está completo. **Fase 9 (cierre de calidad) en curso:** puerta lint/tipos/tests/build en verde y E2E de onboarding (ver [[FASE9-VERIFICATION-EVIDENCE]]); quedan el E2E del ciclo completo, la revisión responsive 390/768/1440 y la fusión a `develop` de las PR #8 y #9. Sigue también C4 (captura de ticket de compra: OCR/importación de líneas detectadas, nodos Figma `COMPRA` 29:3999/29:4136).
