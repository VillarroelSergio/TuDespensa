...
- **Arquitectura:** Fases 4A–4C, 5A, 5B, 5C y 6 cerradas; sigue Fase 7 (cierre de Compra: vista C2 de revisión de productos marcados con previsión de cambios y CTA **Confirmar en despensa**; confirmación idempotente que crea entradas o actualiza presencia en Despensa una sola vez con movimientos trazables; volver desde revisión conserva las marcas y los conflictos/cambios remotos ofrecen recarga sin ocultar el estado visible) según el plan maestro. Figma `COMPRA`, nodo `29:3290`. Las RPC idempotentes de Despensa (`pantry_record_entry`, `pantry_correct_item`) y el patrón `pantry_claim`/`pantry_store_result` son la base a reutilizar.
