...
### 6.1 Incidente de tormenta de mutaciones obsoletas (piloto, 2026-07-28)

`MiDespensa Pilot` registró una tormenta activa de mutaciones fallidas (del orden de miles de rollbacks por segundo, cientos de millones de rollbacks acumulados y millones de intentos de insertar clave idempotente), con el error repetido `Item was changed, unavailable, or inaccessible` en `shopping_toggle_item` y `pantry_adjust_item`. La base tenía muy pocos datos: no era un problema de escala.

**Diagnóstico:** las RPC con control optimista de versión reclamaban (`INSERT`) la clave idempotente antes de comprobar la versión. Una petición con versión obsoleta —doble clic, reintento, edición concurrente— insertaba la clave, fallaba la comprobación de versión justo después y el `RAISE` abortaba toda la transacción. La clave nunca llegaba a persistir, pero el `INSERT` + rollback ya había generado WAL y bloqueado la fila; con solicitudes concurrentes repetidas esto se convierte en una tormenta de escritura sin avance útil. En el cliente, `ShoppingWorkspace` generaba una clave de idempotencia nueva en cada llamada e ignoraba `isPending`, así que un doble clic disparaba dos RPC con dos claves distintas, cada una capaz de reproducir el patrón.

**Decisión:** reordenar el patrón de idempotencia a *hash de la solicitud → resultado idempotente previo (solo lectura, sin insertar) → validar acceso y versión actual → reclamar/insertar la clave → mutar y guardar el resultado*. Una versión obsoleta nunca llega a insertar una clave; un reintento genuino (misma clave, misma carga) se sirve del resultado ya guardado aunque la versión actual haya avanzado por esa misma mutación. Aplicado a `private.pantry_mutate` (cubre `pantry_correct_item`, `pantry_adjust_item`, `pantry_consume_item`, `pantry_mark_low`, `pantry_mark_out`), `shopping_toggle_item`, `pantry_set_attention` y `shopping_remove_item` — las únicas RPC que reproducían el orden defectuoso. `pantry_delete_item` y `pantry_move_item` ya comprobaban la versión antes de reclamar y no se tocaron; las RPC de lote de un único clic (`shopping_confirm_purchase` y variantes, `plan_cook_meal`) no reproducen la tormenta observada y quedan fuera de este hotfix. En el cliente, `ShoppingWorkspace` ahora usa `isPending` para bloquear la interacción mientras una mutación está en curso y un guard síncrono por ítem/lote evita una segunda RPC ante clics rápidos, sin reintento automático en conflicto (`40001`).

**Verificación post-despliegue (2026-07-28):** la primera corrección evitó nuevas inserciones idempotentes, pero las pestañas antiguas siguieron enviando conflictos que abortaban la transacción. Se aplicó una segunda defensa: `shopping_toggle_item` y `private.pantry_mutate` devuelven `{ "status": "conflict" }` en los conflictos esperables de versión o elemento no disponible, sin `RAISE` ni rollback; las server actions actuales lo convierten en `AppError(CONFLICT)`, actualizan la vista y registran una muestra de conflicto como máximo una vez por minuto por operación/ítem/versión. La medición posterior fue concluyente: **0 rollbacks, 0 llamadas nuevas de contexto y 0 inserciones idempotentes en 19,486 s**, frente a ≈2.403 rollbacks/s antes de esta defensa. El cliente bloquea además los clics consecutivos de «se terminó» mediante un guard síncrono, igual que Compra.

**Operación de sesiones:** revocar sesiones invalida los refresh tokens y fuerza la autenticación cuando el JWT actual expire; no revoca de forma instantánea un access token ya emitido. Solo se ejecutará como acción administrativa explícita y con alcance confirmado (una cuenta o todas las del piloto), pues interrumpe a los usuarios.
