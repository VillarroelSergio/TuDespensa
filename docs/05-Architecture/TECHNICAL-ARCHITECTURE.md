...
**Plan de medición post-despliegue:** en el panel de Supabase (`Database` → `Query Performance` / estadísticas de la base), comparar antes/después del despliegue: tasa de rollback por segundo, número de filas insertadas en `idempotency_keys` por minuto y frecuencia del error `Item was changed, unavailable, or inaccessible` en los logs de RPC. Confirmar que el volumen cae a un nivel consistente con uso manual real (interacciones puntuales del hogar piloto, no miles por segundo) durante al menos una sesión de uso normal.
